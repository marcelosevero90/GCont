    ...
    try
      ...
      if sAcao = 'DELETE' then begin
        ...
        if not fqFKTab.IsEmpty then begin
          Result := Result + 'Banco  ' + dsRegistros.FieldByName('codBanco').AsString + ' vinculado a um portador, não pode ser eliminado! |';
        end;
        ...
        fqFKTab.Active := True;
        if not fqFKTab.IsEmpty then begin
          Result := Result + 'Banco  ' + dsRegistros.FieldByName('codBanco').AsString + ' vinculado a um emitente, não pode ser eliminado! |';
        end;
        ...
        if dsRegistros.State = dsInsert then begin
          ...
          if (dsRegistros.FieldByName('codBanco').AsInteger = 0 ) or
            ...
            Result := Result + 'Código do banco deve ser informado! |';
          end;
          ...
          fqFKTab.Active := True;
          if not fqFKTab.IsEmpty then begin
            Result := Result + 'Código ' + dsRegistros.FieldByName('codBanco').AsString + ' já está cadastrado! |';
          end;
        end;
