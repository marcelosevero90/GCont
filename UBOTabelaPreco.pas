    ...
    try
      ...
      if sAcao = 'DELETE' then begin
        ...
        if not fqFKTab.IsEmpty then begin
          Result := Result + 'Tabela de Preço  ' + dsRegistros.FieldByName('codTabPreco').AsString + ' vinculado a um item da tabela, não pode ser eliminado! |';
        end;
        ...
        if dsRegistros.State = dsInsert then begin
          ...
          if (dsRegistros.FieldByName('codTabPreco').AsInteger = 0 ) or
            ...
            Result := Result + 'Código da Tab Preço deve ser informado! |';
          end
          else begin
            fqFKTab.Close;
            fqFKTab.SQL.Clear;
            ...
            fqFKTab.Active := True;
            if not fqFKTab.IsEmpty then begin
              Result := Result + 'Código ' + dsRegistros.FieldByName('codTabPreco').AsString + ' já está cadastrado! |';
            end;
          end;
        end;
        if (dsRegistros.FieldByName('nomeTabPreco').AsString = '' ) or
           (dsRegistros.FieldByName('nomeTabPreco').IsNull)         then begin
          Result := Result + 'Nome da Tab Preço deve ser informado! |';
        end;
        ...
        if (dsRegistros.FieldByName('dtIniValidade').IsNull)         then begin
          Result := Result + 'Início da Validade deve ser informada! |';
        end;
