...
uses
  ...
  function fBOCondPagto_ValidaPostDelete(dsRegistros : TDataSet; sAcao : string ) : string;
      ...
      if sAcao = 'DELETE' then begin
        ...
        if dsRegistros.State = dsInsert then begin
          ...
          if (dsRegistros.FieldByName('codCondPagto').AsInteger = 0 ) or
            ...
            Result := Result + 'Código da Condição de Pagamento deve ser informado! |';
          end;
          ...
          if not fqFKTab.IsEmpty then begin
            Result := Result + 'Código ' + dsRegistros.FieldByName('codCondPagto').AsString + ' já está cadastrado! |';
          end;
        end;
        ...
        if (dsRegistros.FieldByName('numParcelas').AsInteger <= 0 ) or
          ...
          Result := Result + 'Número de parcelas deve ser informado! |';
        end;
