...
begin
  ...
  if fdOrdemServicosituacao.AsInteger > 1 then begin
    MessageDlg('Situação da OS não permite manutenção!',mtError,[mbOK],nil);
    Exit;
  end;
...
begin
  ...
  if Column.FieldName = 'FC_IMG_DEL' then begin
    ...
    if fdOrdemServicosituacao.AsInteger > 1 then begin
      MessageDlg('Situação da OS não permite exclusão!',mtError,[mbOK],nil);
      Exit;
    end;
    ...
    if fdOrdemServicosituacao.AsInteger < 3 then begin
      ...
      if fdOrdemServicosituacao.AsInteger = 2 then begin
        ...
        if not fqAuxiliar.IsEmpty then begin
          MessageDlg('Já existem movimentos financeiros para os títulos da OS, não pode ser cancelada!',mtError,[mbOK],nil);
          Exit;
        end;
...
begin
  ...
  1: DataSet.FieldByName('FC_DSC_TIPO_ORDEM').AsString := 'Serviço';
  end;
  case DataSet.FieldByName('tipoPreco').AsInteger of
  1: DataSet.FieldByName('FC_DSC_TIPO_PRECO').AsString := 'Tabela de Preço';
...
begin
  ...
  fqAuxiliar.SQL.Add(' select * from gcUsuarioPermisPrograma ' +
                     ' where codEmpresa   = ' + IntToStr(MainForm.iCodEmpresa) +
                     '   and codEstabel   = ' + IntToStr(MainForm.iCodEstabel) +
                     '   and codPrograma  = ' + QuotedStr('TIPO PREÇO OS')
                     );
  fqAuxiliar.Active := True;
  if fqAuxiliar.IsEmpty then begin
