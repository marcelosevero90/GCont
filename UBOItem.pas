...
uses
  ...
  function fBOItem_ValidaPostDelete(dsRegistros : TDataSet; sAcao : string ) : string;
      ...
      if sAcao = 'DELETE' then begin
        ...
        if dsRegistros.State = dsInsert then begin
          ...
          if (dsRegistros.FieldByName('codItem').AsString = '' ) or
            ...
            Result := Result + 'Código do Item deve ser informado! |';
          end
          else begin
            fqFKTab.Close;
            fqFKTab.SQL.Clear;
            ...
            if not fqFKTab.IsEmpty then begin
              Result := Result + 'Código ' + dsRegistros.FieldByName('codItem').AsString + ' já está cadastrado! |';
            end;
          end;
        end;
        if (dsRegistros.FieldByName('descricao').AsString = '' ) or
           (dsRegistros.FieldByName('descricao').IsNull)         then begin
          Result := Result + 'Descriação deve ser informada! |';
        end;

        if (dsRegistros.FieldByName('tipoControle').AsInteger = 0 ) or
           (dsRegistros.FieldByName('tipoControle').IsNull)         then begin
          Result := Result + 'Tipo Controle deve ser informado! |';
        end;
