...
begin
  ...
  try
    ...
    if iAcao = 1 then
      fdTitulosMovtoAReceberdescricao.AsString          := 'Implantação'
    else
    if iAcao = 2 then
...
begin
  ...
  if not fValidaPermisManutAR then begin
    MessageDlg('Você não possuí permissão para manutenção em Títulos a Receber!',mtError,[mbOK],nil);
    Exit;
  end;
...
begin
  if Column.FieldName = 'FC_IMG_EDIT' then begin
    ...
    if fdTitulosARecebersituacao.AsInteger > 1 then begin
      MessageDlg('Situação do Título a Receber não permite manutenção!',mtError,[mbOK],nil);
      Exit;
    end;
  ...
  if Column.FieldName = 'FC_IMG_DEL' then begin
    ...
    if fdTitulosARecebersituacao.AsInteger > 1 then begin
      MessageDlg('Situação do Título a Receber não permite exclusão!',mtError,[mbOK],nil);
      Exit;
    end;
    ...
    MessageDlg('Tem certeza que deseja eliminar o Título abaixo?' + chr(10) + fdTitulosARecebercodTituloAReceber.AsString  ,mtConfirmation,mbYesNo, pDeleteConfirmar);
  ...
  if Column.FieldName = 'FC_IMG_CANCEL' then begin
    ...
    MessageDlg('Tem certeza que deseja Cancelar o Título abaixo?' + chr(10) + fdTitulosARecebercodTituloAReceber.AsString  ,mtConfirmation,mbYesNo, pCancelarConfirmar);
  ...
  if Column.FieldName = 'FC_IMG_ESTORNO' then begin
    ...
    if fdTitulosARecebersituacao.AsInteger <> 9 then begin
      MessageDlg('Situação do Título a Receber não permite Estorno!',mtError,[mbOK],nil);
      Exit;
    end;


    MessageDlg('Tem certeza que deseja Estornar o Título abaixo?' + chr(10) + fdTitulosARecebercodTituloAReceber.AsString  ,mtConfirmation,mbYesNo, pEstornoConfirmar);
...
begin
  ...
  fqAuxiliar.SQL.Add(' select codPrograma from gcUsuarioPermisPrograma ' +
                     ...
                     '   and codPrograma = ' + QuotedStr('PERMITE MANUTENÇÃO AR')
                     );
  fqAuxiliar.Active := True;
  if not fqAuxiliar.IsEmpty then
    Result := True;
...
begin
  if AResult = mrYes then begin
    ...
    FCaixaTextoGlobal.Caption := 'Cancelamento Título AR';
