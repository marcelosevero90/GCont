...
begin
  if fdOrdemManutencaosituacao.AsInteger > 1 then begin
    MessageDlg('Situação da OM não permite manutenção!',mtError,[mbOK],nil);
    Exit;
  end;
...
begin
  ...
  if Column.FieldName = 'FC_IMG_DEL' then begin
    ...
    if fdOrdemManutencaosituacao.AsInteger > 1 then begin
      MessageDlg('Situação da OM não permite exclusão!',mtError,[mbOK],nil);
      Exit;
    end;
  ...
  if Column.FieldName = 'FC_IMG_CANCELAOS' then begin
    ...
    if fqAuxiliar.IsEmpty then begin
      MessageDlg('Usuário não possuí permissão para Cancelar OM!' ,mtError,[mbOK],nil);
      Exit;
    end;
    ...
    if fdOrdemManutencaosituacao.AsInteger < 3 then begin
      ...
      if fdOrdemManutencaosituacao.AsInteger = 2 then begin
        ...
        if not fqAuxiliar.IsEmpty then begin
          MessageDlg('Já existem movimentos financeiros para os títulos da OM, não pode ser cancelada!',mtError,[mbOK],nil);
          Exit;
        end;
        sMsgAux := chr(10)+  'Atenção - Títulos financeiros serão eliminados!';
      end;
  ...
  if Column.FieldName = 'FC_IMG_EFETIVA' then begin
    ...
    fqAuxiliar.Active := True;
    if fqAuxiliar.IsEmpty then begin
      MessageDlg('Usuário não possuí permissão para Efetivar OM!' ,mtError,[mbOK],nil);
      Exit;
    end;
...
begin
  ...
  2: DataSet.FieldByName('FC_DSC_SIT').AsString := 'Concluída';
...
begin
  ...
  if AResult = mrYes then begin
    ...
    MessageDlg('Processo concluído com sucesso!' ,mtInformation,[mbOK],nil);
  end;
  ...
  try
    ...
    try
      ...
      if sItensCarrega <> '' then begin
        MessageDlg('Confirma inclusão do(s) item(s) abaixo?' + chr(10) + sItensCarrega ,mtConfirmation,mbYesNo,pConfirmaIncItXML);
      end
      else begin
        MessageDlg('Não foram localizados itens, verifique se o EAN está devidamente cadastro no item e tente novamente!' ,mtInformation,[mbOK],nil);
